...
static Image parse_pixel_data(int width, int height, uint16_t maxval,
                              std::ifstream* file) {
...
static Image load_ppm(const std::string& path) {
    ...
    file >> width;
    if (!file)
        throw std::runtime_error("File's value for `width` was invalid.");

    file >> height;
    if (!file)
        throw std::runtime_error("File's value for `height` was invalid.");
    ...
    Image img = parse_pixel_data(width, height, maxval, &file);
...
static int show_image(const Image& img, std::string file_name) {
    ...
    SDL_Window* window =
        SDL_CreateWindow(file_name.c_str(), img.width, img.height, 0);
...
int main(int argc, char* argv[]) {
    ...
    Image img;
    try {
        img = load_ppm(file_name);
    } catch (const std::runtime_error& e) {
        std::cerr << "Runtime error: " << e.what() << std::endl;
        return 1;
    }

    show_image(img, file_name);
    return 0;
}